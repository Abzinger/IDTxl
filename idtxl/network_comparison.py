...
class NetworkComparison(NetworkAnalysis):
    ...
    def _calculate_cmi_diff_within(self, data_a, data_b):
        ...
        self.cmi_a = self._calculate_cmi_all_links(data_a)
        self.cmi_b = self._calculate_cmi_all_links(data_b)
        self.cmi_diff = self._calculate_diff()
        # Compare raw TE values between conditions.
        self.cmi_comp = self._compare_union_cmi_within()
    ...
    def _calculate_cmi_diff_between(self, data_set_a, data_set_b):
        ...
        # Compare raw TE values between conditions.
        self.cmi_comp = self._compare_union_cmi_between()
    ...
    def _calculate_diff_of_mean(self, permute=False):
        ...
        if permute:
            ...
            # calculating the CMI.
            cmi_all = np.array(self.cmi_a + self.cmi_b)
            new_partition_a = np.random.choice(range(len(cmi_all)),
                                               size=len(self.cmi_a),
                                               replace=False)
            new_partition_b = np.array(list(set(range(0, len(cmi_all))) -
                                            set(new_partition_a)))
            cmi_a_perm = cmi_all[new_partition_a]
            cmi_b_perm = cmi_all[new_partition_b]
    ...
    def _compare_union_cmi_between(self):
        ...
        for t in self.union.targets_analysed:
            cmi_comp[t] = cmi_a_mean[t] > cmi_b_mean[t]
        return cmi_comp

    def _compare_union_cmi_within(self):
        """Compare TE between conditions to get direction of effect."""
        cmi_comp = {}
        for t in self.union.targets_analysed:
            cmi_comp[t] = self.cmi_a[t] > self.cmi_b[t]
        return cmi_comp
    ...
    def _calculate_diff(self):
        ...
        cmi_diff = {}
        for t in self.union.targets_analysed:
            cmi_diff[t] = self.cmi_a[t] - self.cmi_b[t]
        return cmi_diff
    ...
    def _create_surrogate_distribution_between(self):
        ...
        for p in range(self.settings['n_perm_comp']):
            surrogate = self._calculate_diff_of_mean(permute=True)
            for t in self.union.targets_analysed:
    ...
    def _p_value_union(self):
        ...
        for t in self.union.targets_analysed:
            ...
            if not sources.size:
                continue
            self.significance[t] = np.zeros(len(sources))
